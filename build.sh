...
for table_name in $(toml_get_table_names); do
	...
	for dl_from_key in "${DL_SRCS[@]}"; do
		if app_args[${dl_from_key}_dlurl]=$(toml_get "$t" "${dl_from_key}-dlurl"); then
			app_args[${dl_from_key}_dlurl]=${app_args[${dl_from_key}_dlurl]%/}
			app_args[${dl_from_key}_dlurl]=${app_args[${dl_from_key}_dlurl]%download}
			app_args[${dl_from_key}_dlurl]=${app_args[${dl_from_key}_dlurl]%/}
			app_args[dl_from]=${dl_from_key}
		else
			app_args[${dl_from_key}_dlurl]=""
		fi
	done
	...
	app_args[module_prop_name]=$(toml_get "$t" module-prop-name) || app_args[module_prop_name]="${table_name_f}-morphe"
...
log "\nInstall [Morphe MicroG-RE](https://github.com/MorpheApp/MicroG-RE/releases/latest) for non-root YouTube, YT Music and Reddit APKs"
log "Use [zygisk-detach](https://github.com/j-hc/zygisk-detach/releases/latest) to detach root YouTube and YT Music modules from Play Store"
