...
pr() { echo -e "\033[0;32m[+] ${1}\033[0m" >&2; }
epr() {
...
patches_list_versions() {
	...
	cmd="java -jar '$cli_jar' list-versions --patches='$patches_jar' -f '$pkg_name'"
...
merge_splits() {
	...
	if ! OP=$(java -jar "$APKSIGNER" sign --ks ks.keystore --ks-pass pass:123456789 --key-pass pass:123456789 --ks-key-alias jhc \
...
apkmirror_search() {
	...
	local node app_table dlurl="" emptyCheck
...
patch_apk() {
	...
	local cmd="java -jar '$cli_jar' patch '$stock_input' --disable-purge -o '$patched_apk' -p '$patches_jar' --keystore=ks.keystore \
